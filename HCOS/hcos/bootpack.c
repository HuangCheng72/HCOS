...
void HariMain(void){
    ...
    //鼠标的x，y坐标，以及变量i存放从缓冲区中读取的数据，后面这些不太清楚。。。
	...
	//buf_mouse就是之前的mcursor，背景，鼠标的绘制数据
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	if (finfo != 0) {
		...
		for (i = 16 * 256; i < 16 * 256 + 32 * 94 * 47; i++) {
			nihongo[i] = 0xff; //没有字库，全角部分全部以0xff填充
		}
	}
	...
	*((int *) 0x0ff0) = (int) hzk16;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			//如果缓冲区中没有数据，又存在还没有执行的绘图操作就立即执行（或者是刷新图层）
			if (new_mx >= 0) {
				io_sti();
			...
			if (key_win != 0 && key_win->flags == 0) {	//输入窗口被关闭
				if (shtctl->top == 1) {	//当画面上只剩下鼠标和背景的时候
					key_win = 0;
				} else {
			...
			if (256 <= i && i <= 511) { //键盘数据 
				...
				if (i == 256 + 0x3b && key_shift != 0 && key_win != 0) {	//Shift+F1
					...
					if (task != 0 && task->tss.ss0 != 0) {
						...
						io_sti();
						task_run(task, -1, 0);	//为了确实执行结束处理，如果任务处于休眠状态就唤醒
					}
				}
				if (i == 256 + 0x3c && key_shift != 0) {	// Shift+F2
					//自动将输入焦点切换到新打开的命令行窗口（比较方便）
					if (key_win != 0) {
						keywin_off(key_win);
					}
					key_win = open_console(shtctl, memtotal);
