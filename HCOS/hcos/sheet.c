...
struct SHTCTL *shtctl_init(struct MEMMAN *memman, unsigned char *vram, int xsize, int ysize){
	...
	for (i = 0; i < MAX_SHEETS; i++) {
		...
		ctl->sheets0[i].ctl = ctl; //我们申请的ctl 
	}
err:
	return ctl;
}
//在sheets0数组里面寻找未使用的图层，如果找到了就将其标记为正在使用，然后返回地址 
...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0){
	...
	for (h = h0; h <= ctl->top; h++) {
		sht = ctl->sheets[h];
		sid = sht - ctl->sheets0; //将进行了减法运算的地址作为图层号码（编号）使用
		buf = sht->buf;
		bx0 = vx0 - sht->vx0;
		by0 = vy0 - sht->vy0;
		bx1 = vx1 - sht->vx0;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1){
	...
	for (h = h0; h <= h1; h++) {
		...
		if ((sht->vx0 & 3) == 0) {
			...
			for (by = by0; by < by1; by++) {
				vy = sht->vy0 + by;
				for (bx = bx0; bx < bx1 && (bx & 3) != 0; bx++) {	//前面被4除多余的部分逐个字符写入
					vx = sht->vx0 + bx;
				...
				for (bx += i1 * 4; bx < bx1; bx++) {				//后面被4除多余的部分逐个字符写入
					vx = sht->vx0 + bx;
					if (map[vy * ctl->xsize + vx] == sid) {
						vram[vy * ctl->xsize + vx] = buf[by * sht->bxsize + bx];
					}
...
void sheet_updown(struct SHEET *sht, int height){
	...
	if (old > height) {	// 比以前低
		if (height >= 0) {
			...
			ctl->top--; // 正在显示的图层减少了一个，所以最上面的高度减去1
		...
		if (old >= 0) {
			...
			ctl->sheets[height] = sht;
			ctl->top++; // 显示中的图层增加了一个所以最上面的高度也要增加
		}
...
void sheet_free(struct SHEET *sht){
	if (sht->height >= 0) {
		sheet_updown(sht, -1); // 表示如果处于显示状态，则先设定为隐藏
	}
	sht->flags = 0; // 未使用的标志
	return;
}
